...
[Authorize("")]
[Route("v1/entry")]
public sealed class UpdateEntryV1Controller: ControllerBase
{
    private readonly IEntryService _service;

    public UpdateEntryV1Controller(IEntryService service)
    {
        if (service is null)
        {
    ...
    public async Task<IActionResult> Create([FromBody] Entry entry, CancellationToken cancellationToken)
    {
        ...
        await _service.Update(GetAuthenticationContext(), entry, cancellationToken);
