...
/// <summary>
/// </summary>
internal sealed class AuthorizeRequestJsonV1Transcoder: BaseJsonTranscoder<AuthorizeRequest>, ITranscoder
{
    ...
    public override JsonObject Encode(AuthorizeRequest resource) =>
        new JsonObjectBuilder()
            .With("token", resource.Token)
            .With("permission", resource.Permission)
            .Build();

    public override AuthorizeRequest Decode(JsonObject jsonObject) =>
        new AuthorizeRequest()
        {
            Token = GetValue(jsonObject, "token", string.Empty),
            Permission = GetValue(jsonObject, "permission", string.Empty)
        };
}